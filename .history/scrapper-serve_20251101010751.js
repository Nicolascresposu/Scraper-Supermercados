...
async function saveProduct(product, supermarket) {
  ...
  const checking = await poolTimeBased.query('SELECT product_id FROM product_data WHERE product_id = $1', [product_id]); 

  if (result.rowCount === 0) {
    await poolTimeBased.query(sqlProductData_Corregido, [product_id, nombre, supermercado]);    
  }

  try {
    await poolTimeBased.query(sqlTimeBased, [product_id, precio]);
  } catch (err) {
